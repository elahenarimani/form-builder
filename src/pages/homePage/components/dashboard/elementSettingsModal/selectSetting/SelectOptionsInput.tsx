...
const SelectOptionsInput = ({ modalElement, setModalElement }: Props) => {
  function handleChangeOption(index: number, value: string) {
    const updatedOptions = [...modalElement.options]
  ...
  function handleRemoveOption(index: number) {
    ...
    setModalElement({ ...modalElement, options: updatedOptions })
  }
  function handleAddOption() {
    setModalElement({ ...modalElement, options: [...modalElement.options, ""] })
  }
  return (
    <div className="select-options-wrapper w-full h-full  flex flex-col justify-between">
      ...
      <div className="options w-full h-full border-[1px]  rounded-[5px] flex flex-col justify-start items-center  mb-2 p-[18px] border-gray-300">
        ...
        <div className="w-full  flex flex-col justify-start items-start ">
          {modalElement.options.map((opt: string | number, index: number) => (
            <div
              key={index}
            ...
            >
              <Input
                valueState={opt}
                className="flex-1  max-w-[80%]  h-[40px] border border-gray-300 px-2 rounded outline-none focus:ring-1 focus:ring-[#1ABC9C]"
                type="text"
                aria-label="select options"
                placeholder="Enter select Options..."
                onChangeHandler={(e: any) =>
                  handleChangeOption(index, e.target.valueState)
                }
              />
              <Button
